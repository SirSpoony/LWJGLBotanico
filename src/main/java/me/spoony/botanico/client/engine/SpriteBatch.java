...
public class SpriteBatch {
  ...
  public SpriteBatch(int size) {
    Preconditions.checkArgument(Range.closed(0, 8191 * 7).contains(size),
    ...
    shaderProgram.pointVertexAttribute("position", 3, 9 * Float.BYTES, 0);

    shaderProgram.createVertexAttribute("color");
    shaderProgram.enableVertexAttribute("color");
    shaderProgram.pointVertexAttribute("color", 4, 9 * Float.BYTES, 3 * Float.BYTES);

    shaderProgram.createVertexAttribute("texcoord");
    shaderProgram.enableVertexAttribute("texcoord");
    shaderProgram.pointVertexAttribute("texcoord", 2, 9 * Float.BYTES, 7 * Float.BYTES);
  ...
  public void sprite(float x, float y, float width, float height, float rot, float originx,
      float originy, Color color, int texx, int texy, int texwidth, int texheight, Texture tex) {
    if (!rendering) {
      throw new IllegalStateException("Attempted to render sprite without calling being()");
    }
/*    if (z < -1 || z > 1) {
      throw new RuntimeException("Cannot render sprite because z is out of bounds: " + z);
    }*/
    ...
    vertices.put(blx).put(bly).put(0).put(r).put(g).put(b).put(a).put(s1).put(t1);
    vertices.put(ulx).put(uly).put(0).put(r).put(g).put(b).put(a).put(s1).put(t2);
    vertices.put(urx).put(ury).put(0).put(r).put(g).put(b).put(a).put(s2).put(t2);

    vertices.put(blx).put(bly).put(0).put(r).put(g).put(b).put(a).put(s1).put(t1);
    vertices.put(urx).put(ury).put(0).put(r).put(g).put(b).put(a).put(s2).put(t2);
    vertices.put(brx).put(bry).put(0).put(r).put(g).put(b).put(a).put(s2).put(t1);

    vertindex += 6;
  }

  public void sprite(float x, float y, float width, float height, Color color, int texx,
      int texy,
      int texwidth, int texheight, Texture tex) {
    if (!rendering) {
      throw new IllegalStateException("Attempted to render sprite without calling being()");
    }
/*    if (z < -1 || z > 1) {
    ...
    vertices.put(tx1).put(ty1).put(0).put(r).put(g).put(b).put(a).put(s1).put(t1);
    vertices.put(tx1).put(ty2).put(0).put(r).put(g).put(b).put(a).put(s1).put(t2);
    vertices.put(tx2).put(ty2).put(0).put(r).put(g).put(b).put(a).put(s2).put(t2);

    vertices.put(tx1).put(ty1).put(0).put(r).put(g).put(b).put(a).put(s1).put(t1);
    vertices.put(tx2).put(ty2).put(0).put(r).put(g).put(b).put(a).put(s2).put(t2);
    vertices.put(tx2).put(ty1).put(0).put(r).put(g).put(b).put(a).put(s2).put(t1);
  ...
  public void flush() {
    ...
    projectionMatrix.identity().setOrtho(0, viewwidth, 0, viewheight, -1, 1);
  ...
  private static Vector2f rotate(Vector2f center, Vector2f vector, float rot) {
    float x = center.x + (vector.x - center.x) * (float) Math.cos(rot)
        - (vector.y - center.y) * (float) Math.sin(rot);
    float y = center.y + (vector.x - center.x) * (float) Math.sin(rot)
        + (vector.y - center.y) * (float) Math.cos(rot);
