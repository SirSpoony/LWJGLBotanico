...
public class RendererGUI {
    ...
    public GuiRectangle getTextBounds(String text) {
        if (Strings.isNullOrEmpty(text)) return new GuiRectangle();
    ...
    public void text(GuiPosition pos, String text, TextColors textColor, CallAlign callAlign) {
        ...
        switch (callAlign) {
            case BOTTOM_CENTER:
                text(new GuiPosition(pos.x - (int) Math.floor(getTextBounds(text).width / 2), pos.y + getTextBounds(text).height), text, textColor, CallAlign.TOP_LEFT);
                break;
            case BOTTOM_LEFT:
                text(new GuiPosition(pos.x, pos.y + getTextBounds(text).height), text, textColor, CallAlign.TOP_LEFT);
                break;
            case BOTTOM_RIGHT:
                text(new GuiPosition(pos.x - getTextBounds(text).width, pos.y + getTextBounds(text).height), text, textColor, CallAlign.TOP_LEFT);
                break;
            case TOP_RIGHT:
                text(new GuiPosition(pos.x - getTextBounds(text).width, pos.y), text, textColor, CallAlign.TOP_LEFT);
                break;
            case TOP_LEFT:
