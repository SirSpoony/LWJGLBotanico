...
public class BuildingBreakParticles {
  ...
  public void start(TilePosition position, BuildingBreakMaterial mat) {
    ...
    synchronized (bbps) {
      for (int i = 0; i < 6; i++) {
        ...
        bbps.add(new BuildingBreakParticle(region, tempx, tempy, posRand.nextFloat() * 2 - 1, 1,
            tempy - posRand.nextFloat() / 4f, posRand.nextFloat()));
      }
    }
  }
  ...
  protected class BuildingBreakParticle {
    ...
    public BuildingBreakParticle(IntRectangle region, double x, double y, float velx, float vely,
        double floor, float maxlife) {
      this.x = x;
      this.y = y;
      this.velx = velx;
      this.vely = vely;
