...
import me.spoony.botanico.common.net.CPacketBuildingInteraction;
import me.spoony.botanico.common.net.CPacketChangeDialog;
import me.spoony.botanico.common.net.CPacketPlayerMove;
...
public class ClientEntityPlayer extends EntityPlayer implements EntityContainer {
    ...
    public void update(float timeDiff, IPlane level) {
        ...
        if (downAnimation == null) loadAnimation();
            ...
            if (b != null && this.canReach(tilePosition.toGamePosition(new GamePosition())))
                if (buildingDamageIndicator == null) {
                    ...
                    if (!(buildingDamageIndicator.tilePosition.equals(tilePosition))) {
                        buildingDamageIndicator = new BuildingDamageIndicator(tilePosition, b.getHardness(level, tilePosition));
                    }
    ...
    public boolean onBinaryInputPressed(BinaryInput binaryInput) {
        ...
        if (binaryInput == Input.BUTTON_LEFT) {
            ...
            if (this.canReach(tilePosition.toGamePosition(new GamePosition()))) {
                if (cursorStack != null && cursorStack.getItem() instanceof ItemBuilding) {
                    ...
                    if (ec.wouldCollide(tilePosition, hypotheticalBuilding)) return false;
        ...
        if (binaryInput == Input.INV_KEY_1) {
            ...
            slotToExchange = 6;
        } else if (binaryInput == Input.INV_KEY_SWORD) {
            slotToExchange = EntityPlayer.SLOT_SWORD;
        } else if (binaryInput == Input.INV_KEY_HOE) {
            slotToExchange = EntityPlayer.SLOT_HOE;
        } else if (binaryInput == Input.INV_KEY_AXE) {
            slotToExchange = EntityPlayer.SLOT_AXE;
        } else if (binaryInput == Input.INV_KEY_PICKAXE) {
            slotToExchange = EntityPlayer.SLOT_PICKAXE;
        }
    ...
    public void render(RendererGame rg) {
        ...
        rg.sprite(new GamePosition(position).add(0,-2/16f), spritesheet, new IntRectangle(64, 0, 16, 32), position.y+1);
    ...
    public void openInventoryDialog() {
        client.sendPacket(new CPacketChangeDialog().setOperation(CPacketChangeDialog.OPEN_PLAYER_INVENTORY));
    }
