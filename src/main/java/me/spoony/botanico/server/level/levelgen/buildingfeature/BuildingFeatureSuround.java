...
public class BuildingFeatureSuround implements BuildingFeature
{
    ...
    public BuildingFeatureSuround(Building toSurround, float popularity, Building building, Tile allowedTile, int size)
    {
    ...
    public void generate(Random random, long seed, boolean[][] biome, Chunk chunk)
    {
        for (int xi = 0; xi < Chunk.CHUNK_SIZE; xi++)
        {
            for (int yi = 0; yi < Chunk.CHUNK_SIZE; yi++)
            {
                ...
                if (val < popularity && chunk.tiles[xi][yi] == allowedTile && chunk.buildings[xi][yi] == toSurround) {
                    ...
                    for (int i = 0; i < chainlength; i++)
                    {
                        ...
                        if ((chunk.tiles[xmod][ymod] == allowedTile || allowedTile == null) && (chunk.buildings[xmod][ymod] == null))
                        {
                            chunk.buildings[xmod][ymod] = building;
                        }
                    }
                }
            }
