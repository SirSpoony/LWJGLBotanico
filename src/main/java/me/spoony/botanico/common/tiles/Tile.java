...
public class Tile
{
    ...
    private static Cache<Integer, IntRectangle> renderCache;

    static
    {
        renderCache = CacheBuilder.newBuilder().maximumSize(1000).build();
    }
    ...
    public void render(RendererGame rg, final int x, final int y, AdjacentTiles adj)
    {
        final int randtexture = perm[Math.abs(5 * (int) x + (int) y) & 255];

        final TileRenderRule region = renderRules.findRenderRule(adj);

        IntRectangle foreground = null;
        IntRectangle background = null;

        foreground = renderCache.getIfPresent(Objects.hashCode(x, y));
        if (foreground == null)
        {
            if (region == null)
            {
                foreground = renderRules.defaultRegions[randtexture % renderRules.defaultRegions.length];
            } else
            {
                foreground = region.getForeground(randtexture);
            }
            renderCache.put(Objects.hashCode(x, y), foreground);
        }
        ...
        if (background != null)
        {
            rg.sprite(new GamePosition(x, y), rg.getResourceManager().getTexture("tiles.png"),
    ...
    private class AdjacentTilesAndPosition
    {
        public final int x;
        public final int y;

        public AdjacentTilesAndPosition(int x, int y)
        {
            this.x = x;
            this.y = y;
        }
        ...
        {
            if (obj instanceof AdjacentTilesAndPosition == false) return false;
            AdjacentTilesAndPosition atap = ((AdjacentTilesAndPosition)obj);
            if (atap.x != x) return false;
            if (atap.y != y) return false;
            return true;
        }
