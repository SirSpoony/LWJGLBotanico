...
public class ItemTool extends Item {
    ...
    public ItemTool(int id, IntRectangle texturebounds, String name, ItemToolType toolType, float toolPower) {
            ...
            if (level instanceof ServerPlane) {
                ...
                for (int ox = -2; ox <= 2; ox++) {
                    for (int oy = -2; oy <= 2; oy++) {
                        Tile otile = level.getTile(new TilePosition(position).add(ox, oy));
                        if (otile != Tiles.GROUND && otile != Tiles.FERTILIZED_GROUND) valid = false;
                    }
                }
                if (!valid) {
