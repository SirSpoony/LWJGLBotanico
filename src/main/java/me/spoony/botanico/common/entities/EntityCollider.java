...
public class EntityCollider
{
    ...
    public CollisionCheck checkCollisionsChunk()
    {
        ...
        for (int offx = -3; offx < 3; offx++)
            ...
            {
                TilePosition offsetPosition = new TilePosition(new GamePosition(entityBounds.x + offx, entityBounds.y + offy));

                Building b = level.getBuilding(offsetPosition);
                if (b == null) continue;
                if (!b.shouldCollide()) continue;

                DoubleRectangle buildingBounds = new DoubleRectangle(
                        offsetPosition.x+b.getCollisionBounds().x, offsetPosition.y+b.getCollisionBounds().y,
        ...
        for (int offx = -3; offx < 3; offx++)
            for (int offy = -3; offy < 3; offy++)
            {
                ...
                if (t == null) continue;
                if (!t.shouldCollide()) continue;

                DoubleRectangle tileBounds = new DoubleRectangle(offsetPosition.x, offsetPosition.y, 1, 1);
    ...
    public List<Entity> checkCollisionsEntities()
    {
        ...
        {
            DoubleRectangle collisionBounds = new DoubleRectangle(entity.position.x + entity.collider.x, entity.position.y + entity.collider.y,
    ...
    public boolean wouldCollide(TilePosition position, Building b)
    {
        if (!b.shouldCollide()) return false;
        DoubleRectangle buildingBounds = new DoubleRectangle(
