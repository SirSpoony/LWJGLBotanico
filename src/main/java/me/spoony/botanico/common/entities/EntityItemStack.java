...
public class EntityItemStack extends Entity {
    ...
    public EntityItemStack(GamePosition position, IPlane plane, ItemStack stack, boolean randomPosInBlock) {
        super(plane);
    ...
    public void update(float delta, IPlane level) {
        ...
        if (!isBeingCollected()) {
            if (!level.isLocal()) {
                EntityCollider collider = new EntityCollider(level, this, new DoubleRectangle(-1d, -1d, 3d, 3d));
    ...
    public void receiveState(int state) {
        if (state == 0) return; // the state is 0 if there is no collector, otherwise the state IS the collector!
