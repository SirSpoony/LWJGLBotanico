 ...
 */
public class BuildingEntityEnergyPipe extends BuildingEntity implements Updatable,
    IEnergyContainer {
    ...
    for (int ox = -1; ox <= 1; ox++) {
      for (int oy = -1; oy <= 1; oy++) {
        ...
        BuildingEntity obe = ((ServerPlane) plane).getBuildingEntity(new OmniPosition(
            PositionType.GAME, position.getX(PositionType.GAME) + ox,
        ...
        if (obe instanceof IEnergyContainer) {
          ...
          if (energyContainer.getEnergyStored() > getEnergyStored()) {
            float transferred = energyContainer
                .extractEnergy(1, getEnergyCapacity() - getEnergyStored());
            energy += transferred;
          }
        }
      }
