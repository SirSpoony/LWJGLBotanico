...
public class BuildingEntityFurnace extends BuildingEntity implements Updatable {
  ...
  public void update(float delta) {
    ...
    if (dialog.progress > 1) {
      ...
      if (dialog.inventory.getStack(DialogFurnace.PRODUCT_SLOT_ID) == null) {
        dialog.inventory.setStack(DialogFurnace.PRODUCT_SLOT_ID, new ItemStack(Items.CHARCOAL));
      } else {
        dialog.inventory.getStack(DialogFurnace.PRODUCT_SLOT_ID).increaseCount(1);
      }
    }

    dialog.updateViewers();
  }
