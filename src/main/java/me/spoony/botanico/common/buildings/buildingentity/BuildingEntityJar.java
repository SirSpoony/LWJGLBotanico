...
public class BuildingEntityJar extends BuildingEntity implements Updatable, IEnergyContainer {
    ...
    public BuildingEntityJar(TilePosition position, IPlane plane) {
    ...
    public void update(float delta) {
        if (energy >= getEnergyCapacity()) return;
        ...
        if (true) {
            ...
            for (int ox = -2; ox <= 2; ox++) {
                for (int oy = -2; oy <= 2; oy++) {
                    Building obuilding = plane.getBuilding(new TilePosition(position).add(ox, oy));
                    if (obuilding == Buildings.MYSTIC_FLOWER) mysticFlowerCount++;
                }
            }

            energy += mysticFlowerCount*.01f;

            if (energy > getEnergyCapacity()) energy = getEnergyCapacity();
        }
    }
